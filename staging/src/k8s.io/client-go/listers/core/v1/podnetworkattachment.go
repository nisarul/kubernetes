...
type PodNetworkAttachmentLister interface {
	...
	List(selector labels.Selector) (ret []*v1.PodNetworkAttachment, err error)
...
// List lists all PodNetworkAttachments in the indexer.
func (s *podNetworkAttachmentLister) List(selector labels.Selector) (ret []*v1.PodNetworkAttachment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.PodNetworkAttachment))
	})
	return ret, err
}
...
type PodNetworkAttachmentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.PodNetworkAttachment, error)
...
// List lists all PodNetworkAttachments in the indexer for a given namespace.
func (s podNetworkAttachmentNamespaceLister) List(selector labels.Selector) (ret []*v1.PodNetworkAttachment, err error) {
...
// Get retrieves the PodNetworkAttachment from the indexer for a given namespace and name.
func (s podNetworkAttachmentNamespaceLister) Get(name string) (*v1.PodNetworkAttachment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("podnetworkattachment"), name)
	}
	return obj.(*v1.PodNetworkAttachment), nil
}
